...
bool MainWindow::maybeSave()
{
    if(area->isModified())
    {
        ...
        box = QMessageBox::warning(this,tr("Save"),tr("Picture is been change, do you want to save it?"),
...
void MainWindow::creatColorComboBox(QComboBox *comboBox)
{
    ...
    comboBox->addItem(QIcon(pix),tr("black"),QColor(Qt::black));//再用该图片作为组合框条目的图标
    painter.fillRect(0,0,16,16,Qt::white);
    comboBox->addItem(QIcon(pix),tr("white"),QColor(Qt::white));
    painter.fillRect(0,0,16,16,Qt::red);
    comboBox->addItem(QIcon(pix),tr("red"),QColor(Qt::red));
    painter.fillRect(0,0,16,16,Qt::green);
    comboBox->addItem(QIcon(pix),tr("green"),QColor(Qt::green));
    painter.fillRect(0,0,16,16,Qt::blue);
    comboBox->addItem(QIcon(pix),tr("blue"),QColor(Qt::blue));
    painter.fillRect(0,0,16,16,Qt::yellow);
    comboBox->addItem(QIcon(pix),tr("yellow"),QColor(Qt::yellow));

    comboBox->addItem(tr("not"),QColor(Qt::transparent));  //即透明
}
