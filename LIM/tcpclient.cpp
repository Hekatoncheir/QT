...
void TcpClient::readMessage()
{
    ...
    if(bytesReceived <= sizeof(qint64)*2){
        ...
        if((tcpClient->bytesAvailable() >= fileNameSize) && (fileNameSize != 0)){
            ...
            if(!localFile->open(QFile::WriteOnly)){
                QMessageBox::warning(this,tr("APP"),tr("Can't to read the file %1:\n%2.").arg(fileName).arg(localFile->errorString()));
                return;
            }
        }else{
            return;
    ...
    ui->tcpClientStatusLabel->setText(tr("receive %1MB (%2MB/s) \nall%3MB Time:%4秒\nRemaining time:%5秒")
    ...
    {
        tcpClient->close();
        ui->tcpClientStatusLabel->setText(tr("reveive file %1 over").arg(fileName));
    localFile->close();
    }
}
